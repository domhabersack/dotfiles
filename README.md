...
- `npm install -g ccusage` — Claude Code token/cost tracker. `prefix u` opens monthly cost/token totals for (up to) the last six months in a floating popup; `prefix B` opens a companion popup with the aggregated per-model cost/token distribution over the last 30, 60, and 90 days (`bin/tmux-ccusage-popup` and `bin/tmux-ccusage-models-popup`). Not `prefix U` — TPM already binds that to "update plugins".
