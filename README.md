...
- `npm install -g ccusage` — Claude Code token/cost tracker. `prefix u` opens monthly cost/token totals for (up to) the last six months in a floating popup; `prefix i` opens a companion popup with the aggregated per-model cost/token distribution for this month (to date) and last month (`bin/tmux-ccusage-popup` and `bin/tmux-ccusage-models-popup`). `prefix i` overrides tmux's default `display-message` binding — a deliberate tradeoff.
